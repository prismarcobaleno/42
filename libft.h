...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/04 16:18:14 by rosmessi          #+#    #+#             */
/*   Updated: 2025/12/04 16:18:17 by rosmessi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*ft_substr(char const *s, unsigned int inizio, unsigned long len);
...
*/
// da qua in poi - tutta disinfestazione 
/*
void colora(const char *str, int colore)
{
...
int	main(int argc, char *argv[])
{
	if (argc > 2)
	{
		colora("\t(po umolcianiju argv[1])", 32);
		printf("alpha: %d\n", ft_isalpha(argv[1][0]));
		
		printf("isdigit(argc): %d\n", ft_isdigit(argc));

		printf("alnum: %d\n", ft_isalnum(argv[1][0]));
		printf("ascii: %d\n", ft_isascii(argv[1][0]));
		printf("print: %d\n", ft_isprint(argv[1][0]));

		printf("toup[1]: %d %d\n", argv[1][1], ft_toupper(argv[1][1]));
		printf("tolow[2]: %d %d\n", argv[1][1], ft_tolower(argv[1][2]));


		printf("len: %d\n", ft_strlen(argv[1]));

		colora("\twrite e printf non vanno tanto d'accordo", 33); 
		// se non metti \n alla fine printf esplode.
		// grazie al cielo colora ce l'ha incluso
		
		write(1, "ft_memset: ", 11);
		write(1, ft_memset(argv[1], argv[2][0], (argc-4)), 10);
		write(1, "\n   memset: ", 13);
		write(1, memset(argv[1], argv[2][0], (argc-4)), 10);
		write(1, "\n", 1);
		
		write(1, "ft_bzero: ", 11);
		memset(argv[1], '1', 9); //resetto
		ft_bzero(argv[1], (argc-4));
		write(1, argv[1], 10);
		
		write(1, "\n   bzero: ", 13);
		memset(argv[1], '3', 9); //resetto
		bzero(argv[1], (argc-4));
		write(1, argv[1], 10);

		write(1, "ft_memcpy: ", 11);
		memset(argv[1], '4', 9); //resetto
		ft_bzero(argv[1], (argc-4));
		write(1, argv[1], 10);
	...
	{
		colora("\teh ma qualcosina dammela. almeno 3 argomenti", 31);
	}
	return (0);
}
